...
<h2 style='text-align:center;margin-top:20px;margin-bottom:20px;'>部门用户列表</h2>
  ...
  <div class="row">
    <div class="col-xs-6 col-md-4">
		...
		action="${pageContext.request.contextPath}/DepmentListController?pageNume=1">
		<input type='text' name='pageNum' value='1' style='display: none;' />
		<input type='text' name='columnName' value='depName'
			style='display: none;' />
		<div class="form-group">
			<label for="exampleInputEmail1">部门名称：</label> <input type="text"
				class="form-control" name='columnValue'
				value='${sessionScope.columnValue }' placeholder="请输入部门名称">
		</div>
	...
	<h3 class="page_h3" align="center">
		<ul class="pagination">
			<c:if test="${requestScope.page.pageNum!=0}">
				<li><a
					href="${pageContext.request.contextPath}/DepmentListController?pageNum=${page.pageNum}&columnName=${sessionScope.columnName}&columnValue=${sessionScope.columnValue}"
			...
			<c:if test="${page.end>0}">
				<c:forEach begin="${page.start}" end="${page.end-1}" var="i"
					...
					<c:if test="${page.pageNum==i}">
						<li class="active"><a
							href="${pageContext.request.contextPath}/DepmentListController?pageNum=${i+1}&columnName=${sessionScope.columnName}&columnValue=${sessionScope.columnValue}">${i+1}</a></li>
					</c:if>
					<c:if test="${page.pageNum!=i}">
						<li><a
							href="${pageContext.request.contextPath}/DepmentListController?pageNum=${i+1}&columnName=${sessionScope.columnName}&columnValue=${sessionScope.columnValue}">${i+1}</a></li>
			...
			<c:if test="${page.end==0}">
				<li class="active"><a
					href="${pageContext.request.contextPath}/DepmentListController?pageNum=1&columnName=${sessionScope.columnName}&columnValue=${sessionScope.columnValue}">1</a></li>
			</c:if>
			...
			<c:if test="${requestScope.page.pageTotal>0 }">
				...
				<c:if
					...
					<li><a
						href="${pageContext.request.contextPath}/DepmentListController?pageNum=${page.pageNum+2}&columnName=${sessionScope.columnName}&columnValue=${sessionScope.columnValue}"
